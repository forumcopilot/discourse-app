...
- **Forum colours.** `ForumTheme` (entered/left by `SingleForumBootstrapPage`) sets `AppTheme.palette` to the open forum's `ForumPalette` (its Discourse light/dark schemes from `DiscourseSiteCapabilities`, remembered in SharedPreferences under `forum_palettes`); `AppTheme.colorSchemeFor` seeds from the forum accent and pins page/text/accent/danger/selected when legible. Use `colorScheme` roles in widgets, and `ForumColors.of(context)` for Discourse-only colours (`love` for likes, `success` for solved, `highlight`). The app-wide change lands a frame after enter/leave (the shell's `Obx` cannot be dirtied mid-build), so the bootstrap page themes itself directly for its first frame. `AppForumConfig.useForumColors` turns it off.
- **Category colours.** Name a category with `CategoryBadge` (or `TopicTaxonomyChips` for category + tags); it looks the style up by id in `DiscourseSiteCapabilities.categoryStyleFor` (per forum, from `/site.json`) and hides Uncategorized. Open a category with `categoryForum(siteContext, id)` so its page gets its colours. Discourse tags have no colour of their own.
