...
///
/// All four of web's entries now render. Links arrived with `linkCount`
/// (canonical SDK `40b5d0b`); the user count is the server's own
/// `participantCount` rather than `participatedUserIds.length`, because
/// that list is a capped posters summary and under-reports on busy topics.
///
/// Still absent, deliberately: **participant avatars**.
/// [FCTopic.participatedUserIds] gives ids but no avatar URLs, and
/// Discourse builds those from a per-user template — rendering them would
/// mean a second fetch per topic.
  ...
  Widget build(BuildContext context) {
    ...
    final entries = <Widget>[
      ...
      if (topic.linkCount > 0)
        _stat(context, Icons.link, topic.linkCount, metaColor),
      if (userCount > 0)
